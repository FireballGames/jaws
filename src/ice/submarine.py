...
def harpoon_hit_func(space, arbiter):
    ...
    #     slidejoint = pymunk.SlideJoint(other_body, harpoon_body.harpoonparent.parent.body, hook_other_point, (0.0, 0.0),
    #                                    0.0, 100.0)
...
def harpoon_hit_monster(space, arbiter):
    ...
    #     harpoon_body.harpoonparent.looseharpoons.append(
    #         LooseHarpoon(harpoon_body.harpoonparent, harpoon_body.harpoonparent.hook,
    #                      harpoon_body.harpoonparent.parent.heading))
...
class LooseHarpoon:
    ...
    def draw(self, screen):
        # ang_ind = int(round((180 * self.body.angle / math.pi + 180) / 10.0)) % 35
        ...
        # screen.blit(sprite_data, (
        # x - (imw / 2) - self.parent.parent.h_camera.x + self.parent.parent.h_camera.w_view / 2,
        # y - (imh / 2) - self.parent.parent.h_camera.y + self.parent.parent.h_camera.h_view / 2))
        pass
...
class Harpoons:
    ...
    def update(self):
        ...
        if self.state == 1 and self.hook_to == 0:  # out of rope, transfer to loose hook list
            self.transfer_to_loose()
        else:
            self.hook_to -= 1

        # look for fire new one
        if self.parent.control.fire == 1 and self.parent.alive and self.fire_to == 0 and self.state == 0:
            self.parent.control.fire = 0
            self.fire_to = 30
    ...
    def draw(self, screen):
        ...
        if self.state > 0:
            if self.state == 1:
                ...
                (imw, imh) = (sprite_data.get_width(), sprite_data.get_height())
                screen.blit(sprite_data, (x - (imw / 2) - self.parent.h_camera.x + self.parent.h_camera.w_view / 2,
                                          y - (imh / 2) - self.parent.h_camera.y + self.parent.h_camera.h_view / 2))
                ...
                ang_ind = int(
                    round((180 * (self.hook_end_data[1].angle + self.hook_end_data[2]) / pi + 180) / 10.0)) % 35
                sprite_data = resources.harpoon_spritedata[ang_ind]
                (imw, imh) = (sprite_data.get_width(), sprite_data.get_height())
                screen.blit(sprite_data, (x4 - (imw / 2) - self.parent.h_camera.x + self.parent.h_camera.w_view / 2,
                                          -y4 - (imh / 2) - self.parent.h_camera.y + self.parent.h_camera.h_view / 2))
...
class Submarine:
    ...
    def set_thought(self, thought, to):
        ...
        self.thought_queue = [i for i in range(len(Resources.thoughtsurfs[thought]))]
        self.thoughtlen = to
        self.thought_to = to

...
class Dart:
    ...
    def Render(self, screen):
        if self.state == 1:
            x1 = 0  # self.body.position[0] - self.h_tiledlayer.h_camera.x + self.h_tiledlayer.h_camera.w_view / 2
            y1 = 0  # -self.body.position[1] - self.h_tiledlayer.h_camera.y + self.h_tiledlayer.h_camera.h_view / 2
        elif self.state == 2:
...
class DartShooter(object):
    ...
    def Update(self):
        ...
        px = self.h_player.body.position[0]
        py = -self.h_player.body.position[1]
        if self.fire_to == 0 and math.fabs(px - self.pos[0]) < 10:
    ...
    def Render(self, screen):
        if self.type == 0:
            tilecoords1 = Resources.tiles_pos[Resources.monster_spriteseq['dartshooter'][0]]
            tilecoords2 = Resources.tiles_pos[Resources.monster_spriteseq['dartshooter'][0] - 16]
            screen.blit(Resources.tiles, (
            self.pos[0] - self.h_tiledlayer.h_camera.x + self.h_tiledlayer.h_camera.w_view / 2,
            self.pos[1] - self.h_tiledlayer.h_camera.y + self.h_tiledlayer.h_camera.h_view / 2), area=tilecoords1)
            screen.blit(Resources.tiles, (
            self.pos[0] - self.h_tiledlayer.h_camera.x + self.h_tiledlayer.h_camera.w_view / 2,
            self.pos[1] - 16 - self.h_tiledlayer.h_camera.y + self.h_tiledlayer.h_camera.h_view / 2), area=tilecoords2)
        elif self.type == 1:
            tilecoords1 = Resources.tiles_pos[Resources.monster_spriteseq['dartshooter'][1]]
            tilecoords2 = Resources.tiles_pos[Resources.monster_spriteseq['dartshooter'][1] + 16]
            screen.blit(Resources.tiles, (
            self.pos[0] - self.h_tiledlayer.h_camera.x + self.h_tiledlayer.h_camera.w_view / 2,
            self.pos[1] - self.h_tiledlayer.h_camera.y + self.h_tiledlayer.h_camera.h_view / 2), area=tilecoords1)
            screen.blit(Resources.tiles, (
...
class Squid(object):
    ...
    def Kill(self):
        ...
        # for i in range(5):
        #     self.h_tiledlayer.bubbles.AddBubble([self.body.position[0] - 24 + random.randint(0, 48),
        #                                          -self.body.position[1] - 24 + random.randint(0, 48)])
    ...
    def Render(self, screen):
        coords2 = Resources.tiles_pos[Resources.monster_spriteseq['squid'][self.frame]]
        coords = list(coords2)
        coords[2] = 32
        coords[3] = 32
        coords = tuple(coords)
        x = 0  # int(self.body.position[0])
        y = 0  # int(-self.body.position[1])
        screen.blit(Resources.tiles, (
        x - (32 / 2) - self.h_tiledlayer.h_camera.x + self.h_tiledlayer.h_camera.w_view / 2,
        y - (32 / 2) - self.h_tiledlayer.h_camera.y + self.h_tiledlayer.h_camera.h_view / 2), area=coords)
...
class Shark(object):
    ...
    def Update(self):
        if self.alive == True:
            ...
            # behaviour
            sx = 0  # self.body.position[0]
            sy = 0  # self.body.position[1]
            ...
            if self.type == 0:
                ...
                for m in self.h_tiledlayer.monsters:
                    if m.is_squid == True and m.alive == False:
                        ...
                        dist2squid = math.sqrt(pow(px2 - sx, 2) + pow(py2 - sy, 2))
                ...
                if closest_squid == 1000:
                    px = self.h_player.body.position[0]
                    py = self.h_player.body.position[1]
                    dist2player = math.sqrt(pow(px - sx, 2) + pow(py - sy, 2))
            else:
                px = self.h_player.body.position[0]
                py = self.h_player.body.position[1]
                dist2player = math.sqrt(pow(px - sx, 2) + pow(py - sy, 2))
            ...
            if ((self.type == 0 and dist2player < 300) or (
                    self.type == 1 and dist2player < 600)) and self.powered == True:  # swimming after player
            ...
            if self.type == 1 and self.powered == True:  # additional roboshark behaviours
                if dist2player < 150 and self.bite_to == 0:  # fire the laser!
                    ...
                    #     self.darts.append(
                    #         Dart([self.body.position[0] - 20, -self.body.position[1]], self.h_tiledlayer, self.h_player,
                    #              2))
                    # else:
                    #     self.darts.append(
                    #         Dart([self.body.position[0] + 20, -self.body.position[1]], self.h_tiledlayer, self.h_player,
    ...
    def Render(self, screen):
        ang_ind = 0  # int(round((180 * self.body.angle / pi + 180) / 10.0)) % 35
        if self.type == 0:
            coords2 = Resources.tiles_pos[Resources.monster_spriteseq['shark'][self.heading][self.frame]]
        elif self.type == 1:
            coords2 = Resources.tiles_pos[Resources.monster_spriteseq['roboshark'][self.heading][self.frame]]
        coords = list(coords2)
        coords[2] = 48
        coords = tuple(coords)
        x = 0  # int(self.body.position[0])
        y = 0  # int(-self.body.position[1])
        screen.blit(Resources.tiles, (
        x - (48 / 2) - self.h_tiledlayer.h_camera.x + self.h_tiledlayer.h_camera.w_view / 2,
        y - (16 / 2) - self.h_tiledlayer.h_camera.y + self.h_tiledlayer.h_camera.h_view / 2), area=coords)
        if self.type == 1:
            for d in self.darts:
                d.Render(screen)

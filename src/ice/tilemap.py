...
from .monsters import DartShooter, Squid, Shark
# import monsters
...
class Bubbles(object):
    ...
    def RenderBubbles(self, screen):
        for b in self.bubbles:
            ...
            screen.blit(Resources.bubbles_spritedata[b.frame], (
            b.pos[0] - self.parent.h_camera.x + self.parent.h_camera.w_view / 2,
            b.pos[1] - self.parent.h_camera.y + self.parent.h_camera.h_view / 2))
...
class FishSchool(object):
    ...
    def Update(self):
        ...
        period2 = 50.0
        if self.type == 0:
            self.pos[0] = self.initpos[0] + 50 * math.sin(float(self.count) / period1) + 10 * math.cos(
                float(self.count) / period2)
            self.pos[1] = self.initpos[1] + 10 * math.cos(float(self.count) / period1) + 5 * math.sin(float(self.count) / period2)
        elif self.type == 1:
            self.pos[0] -= 0.5
            self.pos[1] = self.initpos[1] + 10 * math.cos(float(self.count) / period2)
            if self.pos[0] < 0:
...
class FishSchools(object):
    ...
    def RenderFishSchools(self, screen):
        for s in self.schools:
            ...
            screen.blit(Resources.tiles, (int(s.pos[0]) - self.parent.h_camera.x + self.parent.h_camera.w_view / 2,
                                          int(s.pos[1]) - self.parent.h_camera.y + self.parent.h_camera.h_view / 2),
                        area=tilecoords1)
            screen.blit(Resources.tiles, (
                int(s.pos[0]) + self.parent.tilesize - self.parent.h_camera.x + self.parent.h_camera.w_view / 2,
                int(s.pos[1]) - self.parent.h_camera.y + self.parent.h_camera.h_view / 2), area=tilecoords2)
...
def mine_contact_func(space, arbiter):
    ...
    if mine_body.item_ref.state == 0 and not b.collision_type == 6:
        ...
        # pm # bb = pymunk.BB(position[0] - explode_range, position[1] - explode_range, position[0] + explode_range,
        #    #                position[1] + explode_range)
        bb = 0
        ...
        for shape in bother_list:
            ...
            if math.fabs(relx) == 0:
                ...
                magx = explode_gain * (relx / math.fabs(relx)) * (explode_range - math.fabs(relx))
            if math.fabs(rely) == 0:
                magy = 0
            else:
                magy = explode_gain * (rely / math.fabs(rely)) * (explode_range - math.fabs(rely))
        ...
        for i in range(10):
            mine_body.item_ref.h_tiledlayer.bubbles.AddBubble(
                [position[0] - 50 + random.randint(0, 100), -position[1] - 50 + random.randint(0, 100)])
...
class Mine(object):
    ...
    def Render(self, screen):
        ...
        x1 = 0  # self.body1.position[0] - (16 / 2) - self.h_tiledlayer.h_camera.x + self.h_tiledlayer.h_camera.w_view / 2
        y1 = 0  # -self.body1.position[1] - (16 / 2) - self.h_tiledlayer.h_camera.y + self.h_tiledlayer.h_camera.h_view / 2
        screen.blit(Resources.tiles, (x1, y1), area=tilecoords1)
        if self.state == 0:
            tilecoords2 = Resources.tiles_coords[Resources.item_spriteseq['mine'][1]]
            x2 = 0  # self.body2.position[0] - (16 / 2) - self.h_tiledlayer.h_camera.x + self.h_tiledlayer.h_camera.w_view / 2
            y2 = 0  # -self.body2.position[1] - (16 / 2) - self.h_tiledlayer.h_camera.y + self.h_tiledlayer.h_camera.h_view / 2
...
class Magnet(object):
    ...
    def Update(self):
        ...
        attract_gain = 10.0
        # bb = pymunk.BB(self.body2.position[0] - self.effect_range, self.body2.position[1] - self.effect_range,
        #                self.body2.position[0] + self.effect_range, self.body2.position[1] + self.effect_range)
        bb = None
        ...
        for shape in bother_list:
            ...
            #         range = math.sqrt(pow(self.body2.position[0] - shape.body.position[0], 2) + pow(
            #             self.body2.position[1] - shape.body.position[1], 2))
            ...
            #             range = sqrt(pow(self.body2.position[0] - shape.body.position[0], 2) + pow(
            #                 self.body2.position[1] - shape.body.position[1], 2))
            #             if range > 0 and range < self.effect_range:
            #                 attraction = attract_gain * (self.effect_range - range)
            #                 relx = self.body2.position[0] - shape.body.position[0]
    ...
    def Render(self, screen):
        ...
        x1 = 0  # self.body2.position[0] - (16 / 2) - self.h_tiledlayer.h_camera.x + self.h_tiledlayer.h_camera.w_view / 2
        y1 = 0  # -self.body2.position[1] - (16 / 2) - self.h_tiledlayer.h_camera.y + self.h_tiledlayer.h_camera.h_view / 2
        screen.blit(Resources.tiles, (x1, y1), area=tilecoords1)
...
class TiledLayers(object):
    ...
    def PreRenderTexturedBlocks(self):
        ...
        for b in self.blocks:
            ...
            minx = int(math.floor(min(xvals)))
            miny = int(math.floor(min(yvals)))
            maxx = int(math.ceil(max(xvals)))
            maxy = int(math.ceil(max(yvals)))
            ...
            for i in range(imw / 16 + 1):
                for j in range(imh / 16 + 1):
    ...
    def load_level(self, level_id):
        ...
        # static_body = pymunk.Body()
        # ground = pymunk.Segment(static_body, (self.map_size[0] * ts, 0.0),
        #                         (self.map_size[0] * ts, -self.map_size[1] * ts), 0.0)
        # ground.friction = 0.5
        # self.space.add(ground)
        # static_body = pymunk.Body()
        # ground = pymunk.Segment(static_body, (self.map_size[0] * ts, -self.map_size[1] * ts),
        #                         (0.0, -self.map_size[1] * ts), 0.0)
        # ground.friction = 0.5
        # self.space.add(ground)
        # static_body = pymunk.Body()
        ...
        for i in range(len(self.occlayer)):
        ...
        self.static_ani = []
        for i in range(len(self.effectslayer)):
        ...
        # 5: roboshark
        self.monsters = []
        for i in range(len(self.monsterlayer)):
            if self.monsterlayer[i] == 1:  # laser (up)
                x = self.tilesize * (i % self.map_size[0])
                y = self.tilesize * (i / self.map_size[0])
                self.monsters.append(DartShooter([x, y], self, self.h_player, 0))
            if self.monsterlayer[i] == 2:  # laser (down)
                x = self.tilesize * (i % self.map_size[0])
                y = self.tilesize * (i / self.map_size[0])
                self.monsters.append(DartShooter([x, y], self, self.h_player, 1))
            if self.monsterlayer[i] == 3:  # squid
                x = self.tilesize * (i % self.map_size[0])
                y = self.tilesize * (i / self.map_size[0])
                self.monsters.append(Squid([x, y], self, self.h_player))
            if self.monsterlayer[i] == 4:  # shark
                x = self.tilesize * (i % self.map_size[0])
                y = self.tilesize * (i / self.map_size[0])
                self.monsters.append(Shark([x, y], self, self.h_player, 0))
            if self.monsterlayer[i] == 5:  # robo shark
                x = self.tilesize * (i % self.map_size[0])
                y = self.tilesize * (i / self.map_size[0])
                self.monsters.append(Shark([x, y], self, self.h_player, 1))

        # Read out and assign item
        # 1: mine
        # 2: magnet
        self.items = []
        for i in range(len(self.itemlayer)):
    ...
    def UpdateTileLayer(self, tile, tileval):
        ...
        if tileval >= 0:
            tilecoords = Resources.tiles_pos[tileval]
            self.bglayer_t.blit(Resources.tiles, (coords[0], coords[1]), area=tilecoords)
    ...
    def UpdateTriggers(self):
        for t in self.triggers:
            if t[4] == False:
                ...
                range = math.sqrt(pow(xdiff, 2) + pow(ydiff, 2))
    ...
    def RenderTileLayer(self, screen):
        screen.blit(self.bglayer_t, (0, 0), area=(
        self.h_camera.x - self.h_camera.w_view / 2, self.h_camera.y - self.h_camera.h_view / 2, self.h_camera.w_view,
        self.h_camera.h_view))

    def RenderFGLayer(self, screen):
        screen.blit(self.bglayer_tfg, (0, 0), area=(
        self.h_camera.x - self.h_camera.w_view / 2, self.h_camera.y - self.h_camera.h_view / 2, self.h_camera.w_view,
        self.h_camera.h_view))
    ...
    def RenderStaticAni(self, screen):
        for sa in self.static_ani:
            ...
            tilecoords = Resources.tiles_pos[Resources.static_spriteseq['kelp'][sa.frame]]
            tilecoords = list(tilecoords)
            tilecoords[1] = tilecoords[1] - 1
            tilecoords[3] = tilecoords[3] + 1
            tilecoords = tuple(tilecoords)
            screen.blit(Resources.tiles, (
            sa.x - self.h_camera.x + self.h_camera.w_view / 2, sa.y - 1 - self.h_camera.y + self.h_camera.h_view / 2),
                        area=tilecoords)
    ...
    def RenderBoulders(self, screen):
        for b in self.boulders:
            body = b[4]
            ang_ind = int(round((180 * body.angle / math.pi + 180) / 10.0)) % 35
    ...
    def RenderBlocks(self, screen):
        for b in self.blocks:
            ...
            ang_ind = int(round((180 * angle / math.pi + 180) / 10.0)) % 35
